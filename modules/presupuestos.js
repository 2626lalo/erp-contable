...
const condicionesGenerales = `📌 Condiciones Generales:
• Requiere previa autorización del cliente.
• Valores expresados en Pesos Argentinos (ARS).
• Los precios ${incluirIVA ? 'CONTENEN' : 'NO CONTIENEN'} IVA.
• Los tiempos de entrega dependen de disponibilidad.`;
...
function mostrarEditorPresupuesto() {
        ...
        <div class="fixed inset-0 bg-gray-900 bg-opacity-50 overflow-y-auto z-50 p-4">
            <div class="max-w-6xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-2xl">
                ...
                <div class="p-4">
                    ...
                    <div class="flex gap-4 mb-4 p-3 bg-gray-100 rounded-lg">
                    ...
                    <div id="controlGlobal" class="mb-4 p-3 bg-purple-50 rounded-lg"><label class="block font-medium mb-1">Margen Global: <span id="porcentajeLabel">${markupGlobal}</span>%</label><input type="range" id="markupSlider" min="0" max="200" value="${markupGlobal}" class="w-full"></div>
                    ...
                    <div class="flex gap-3 mb-4"><button id="agregarItem" class="bg-gray-200 px-3 py-1.5 rounded-lg text-sm">+ Agregar item</button><button id="vistaPreviaBtn" class="bg-purple-600 text-white px-4 py-1.5 rounded-lg text-sm ml-auto">👁️ Vista Previa</button></div>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div class="border rounded-lg p-3 bg-gray-50">
                            <h3 class="font-bold text-sm mb-2">📊 Análisis de Incremento</h3>
                            <div id="vistaIncremento" class="text-sm"></div>
                    ...
                    <div class="mb-4 p-3 bg-gray-100 rounded-lg"><label class="block font-medium mb-1">📝 Comentarios</label><textarea id="comentarios" rows="2" class="w-full p-2 border rounded-lg" placeholder="Sin otro particular, quedamos a su disposición..."></textarea></div>
                    
                    <div class="flex gap-3"><button id="guardarPresupuesto" class="flex-1 bg-green-600 text-white py-3 rounded-xl">💾 Guardar</button><button onclick="window.dispatchEvent(new Event('refreshView'))" class="flex-1 bg-gray-300 py-3 rounded-xl">Cancelar</button></div>
                </div>
            </div>
        </div>`;
    ...
    document.getElementById('vistaPreviaBtn').onclick = () => {
        ...
        mostrarVistaPreviaCliente();
    };
    
    function mostrarVistaPreviaCliente() {
        const clienteId = parseInt(document.getElementById('clienteSelect').value);
        const cliente = db.clientes.find(c => c.id === clienteId);
...
• Los tiempos de entrega dependen de disponibilidad.`;
        ...
        previewWindow.document.write(`
            ...
            <body>
                <div class="header"><h1>${empresa?.nombre || 'Mi Empresa'}</h1><p>CUIT: ${empresa?.cuit || '30-12345678-9'} | Tel: ${empresa?.telefono || '11-1234-5678'}</p><p>${empresa?.direccion || 'Av. Corrientes 123, CABA'}</p></div>
                ...
                <div class="footer"><p>${comentarios}</p><p><br>${empresa?.nombre || 'Mi Empresa'} - Saluda atentamente.</p></div>
    ...
    document.getElementById('guardarPresupuesto').onclick = () => {
        ...
        db.presupuestos.push({
            id: generarId(), clienteId: cliente.id, cliente: cliente.nombre,
            ...
            numero, fechaCreacion: new Date().toISOString(), estado: 'PENDIENTE'
        });
        guardarDB();
        mostrarNotificacion(`✅ Presupuesto ${numero} guardado`, 'success');
    ...
    function renderizarTablaItems() {
        ...
        tbody.innerHTML = itemsPresupuesto.map((item, idx) => {
            ...
            const precioVenta = item.costo * (1 + markupItem/100);
            return `
                <tr class="border-b"><td class="p-2 text-center">${item.nro}侧</td>
...
window.verPresupuestoDetalle = (id) => {
    ...
    document.getElementById('root').innerHTML = `
        <div class="bg-white p-5 rounded-2xl shadow-lg pb-24 max-w-4xl mx-auto">
            ...
            <div class="bg-blue-50 p-3 rounded-lg mb-4"><div class="font-bold">${empresa?.nombre || 'Mi Empresa'}</div><div class="text-xs">CUIT: ${empresa?.cuit || '30-12345678-9'}</div></div>
...
• Los tiempos de entrega dependen de disponibilidad.`;
    ...
    const content = `<div style="font-family: Arial; padding: 20px;">
        <div style="text-align: center; border-bottom: 2px solid #1e3a8a;"><h1 style="color:#1e3a8a;">${empresa?.nombre || 'Mi Empresa'}</h1><p>CUIT: ${empresa?.cuit || '30-12345678-9'} | Tel: ${empresa?.telefono || '11-1234-5678'}</p><p>${empresa?.direccion || 'Av. Corrientes 123, CABA'}</p></div>
        ...
        <div style="margin-top:20px; padding:10px; background:#fef3c7; font-size:11px; border-left:4px solid #f59e0b;"><strong>📌 Condiciones Generales:</strong><br>${condiciones.replace(/\n/g, '<br>')}</div>
        <div style="margin-top:30px; font-size:11px; color:#666;"><p>${p.comentarios || "Sin otro particular, quedamos a su disposición."}</p><p><br>${empresa?.nombre || 'Mi Empresa'} - Saluda atentamente.</p></div>
    </div>`;
